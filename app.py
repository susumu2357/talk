...
# def speech_recognize_continuous_from_file(audio: str, language: str):
...
def to_sppech(history: gr.Chatbot, lang: str) -> None:
    speech_config = speechsdk.SpeechConfig(
        subscription=os.environ.get("SPEECH_KEY"),
        region=os.environ.get("SPEECH_REGION"),
    )
...
def compose_messages(history: gr.Chatbot, language: str) -> list[dict[str, str]]:
    ...
    for t in history:
        for i, elm in enumerate(t):
            match i:
                case i if i % 2 == 0:
                    messages += [{"role": "user", "content": elm}]
                case i if i % 2 == 1 and elm:
                    messages += [{"role": "assistant", "content": elm}]
    return messages
...
def user(text: str, history: gr.Chatbot) -> gr.Chatbot:
...
with gr.Blocks() as demo:
    ...
    ).then(to_sppech, [chatbot, lang], html, queue=True).then(
        lambda: [None, None, ""], None, [audio, text, state], queue=False
    )
    ...
    clear.click(lambda: [None, None], None, [chatbot, html], queue=False)
